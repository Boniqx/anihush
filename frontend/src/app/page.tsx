...
export default function Dashboard() {
  ...
  // Real Data
  const { data: rawCompanions, isLoading, error } = useCompanions();

  // Deduplicate companions by name
  const companions = useMemo(() => {
    if (!rawCompanions) return [];
    const unique = new Map();
    rawCompanions.forEach((c) => {
  ...
  useGSAP(
    ...
    { scope: containerRef, dependencies: [isLoading, companions] },
  );
        ...
        <div className="flex-1 flex flex-col min-h-0 space-y-4">
          ...
          <HushBankModal
            ...
            onSuccess={(newBalance) => {
              // Update local state if needed, but optimally we should re-fetch user
              // For now, simple re-fetch or dispatch could work.
              // Assuming parent component or Redux handles user updates or we trigger a refresh.
              // We'll trust the user to refresh or implement a proper profile refresh hook later.
              // Or modify the redux state directly here if we had dispatch accessible easily.
              window.location.reload(); // Simple refresh for now to get new balance
            }}
          />
          ...
          <div className="flex-1 min-h-0 overflow-y-auto no-scrollbar">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 pb-4">
              {isLoading ? (
                <div className="col-span-full flex items-center justify-center py-20 text-[#a0a0b0]">
                  <Sparkles className="w-5 h-5 animate-spin mr-2" /> Loading
                  companions...
                </div>
              ) : error ? (
                <div className="col-span-full py-10 text-center text-[#ff4757] text-sm">
                  Failed to load companion data.
                </div>
          ...
          {/* Logged-out CTA Banner (Only show if space permits or restrict to bottom) */}
          {!isAuthenticated && (
